...
class Utils
{
    ...
    public static function getRouteClassAndMethodNamesForResponse(Route $route): array
    {
        $action = $route->getAction();

        $uses = $action['uses']; // App\Http\Controllers\SalesLocalPaymentController@index

        if ($uses !== null) {
            if (is_array($uses)) {
                return $uses;
            } elseif (is_string($uses)) {
                // App\Http\Controllers\SalesLocalPaymentControllerを
                // App\Http\Response\SalesLocalPayment\SalesLocalPaymentIndexResponseに置換する
                $action = explode('@', $uses);
                $actions = explode('\\', $action[0]);
                $controller = $actions[3]; // SalesLocalPaymentController
                $main = str_replace("Controller", "", $controller); // SalesLocalPayment
                $upperAction = ucfirst($action[1]); // Index
                $response = "$actions[0]\\$actions[1]\\Response\\${main}\\${main}${upperAction}Response";
